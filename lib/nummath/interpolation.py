...
import nummath.linearsystem as linsys
...
class PolyInterPol(_InterPol):
    ...
    def __init__(self, x_data, y_data, method="newton"):
        ...
        Note: Newton's method is preferred if the interpolation needs to be done at multiple points_1.
...
class CubicSplineInterPol(_InterPol):
    ...
    def _curvature(self):
        ...
        self._k = linsys.B3DLinSys(c, d, e, b).solve()
...
class _CurveFit:
    ...
    def plot(self, x_title='x', y_title='y', fig_size=None, dpi=None):
        ...
        """
        if self._solved:
            # calculate some points_1 on the fitting curve
            x = np.linspace(np.min(self._x_data), np.max(self._x_data), 21, endpoint=True)
            y = self.eval_fitting_curve_multi(x)
            # show the data points_1 and the calculated points_1 in a graph
...
class PolyFit(_CurveFit):
    ...
    def solve(self):
        self._c = linsys.GaussElimin(self._a, self._b, pivot_on=True).solve()
        self._solved = True
...
class ExpFit(_CurveFit):
    ...
    """
    # y = c0.e^(c1.x) ==> ln(y) = ln(c0) + c1.x is a straight line and we can use linear regression to find ln(c0)
    # and c1; afterwards we determine c0 from c0 = exp(ln(c0))
